(ns texmachmfg.governor-contract-test
  ...
  mark') implemented faithfully. The single invariant under test:
    ...
    TexMachAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Textile, Apparel and
    Leather Machinery Plant Operations Governor would reject;
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :weaving-loom}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :weaving-loom (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest invalid-no-load-run-speed-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:no-load-run-speed-rpm 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-no-load-run-speed} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:no-load-run-speed-rpm (store/batch db "batch-001"))) "fabricated no-load-run-speed never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "loom-001" :severity :moderate
                                            :description "shuttle vibration anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "loom-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
