(ns texmachmfg.operation-test
  "Smoke tests for the compiled TexMachOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `texmachmfg.governor-contract-test`; the Store contract in
  `texmachmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "TexMachOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "loom-001" :severity :moderate :description "shuttle vibration anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
