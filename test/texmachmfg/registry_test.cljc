(ns texmachmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [texmachmfg.registry :as r]))
...
(deftest known-product-types-are-valid
  (doseq [pt [:weaving-loom :sewing-machine :leather-cutting-machine
...
(deftest typical-no-load-run-speed-is-valid
  (is (r/no-load-run-speed-rpm-valid? 0.0))
  (is (r/no-load-run-speed-rpm-valid? 600.0))
  (is (r/no-load-run-speed-rpm-valid? 5000.0))
  (is (r/no-load-run-speed-rpm-valid? 8000.0)))

(deftest negative-no-load-run-speed-is-invalid
  (is (not (r/no-load-run-speed-rpm-valid? -1.0))))

(deftest excessive-no-load-run-speed-is-invalid
  (is (not (r/no-load-run-speed-rpm-valid? 999999.0)))
  (is (not (r/no-load-run-speed-rpm-valid? 8000.01))))

(deftest non-numeric-or-missing-no-load-run-speed-is-invalid
  (is (not (r/no-load-run-speed-rpm-valid? nil)))
  (is (not (r/no-load-run-speed-rpm-valid? "600"))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "loom-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "loom-001" 1)
