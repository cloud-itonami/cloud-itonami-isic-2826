(ns texmachmfg.store
  "SSoT for the textile/apparel/leather production-machinery plant-
  operations coordination actor, behind a `Store` protocol so the
  backend is a swap, not a rewrite -- the same seam every
  `cloud-itonami-isic-*` actor in this fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-2818`'s own
  `powertoolmfg.store`, `cloud-itonami-isic-2710`'s own
  ...
  is self-contained (no external textile/apparel/leather-machinery-
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/no-load-run-speed/quantity/
    ...
    - `equipment`         -- a weaving-loom/sewing-machine/leather-
                             cutting-machine assembly-or-test-bench
    ...
    - `maintenance`       -- a scheduled assembly/test-bench-equipment
                             ...
                             of equipment (`texmachmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound textile/apparel/
                             leather production-machinery shipment
                             DRAFT (`texmachmfg.registry`'s
                             `register-shipment`).
  ...
  coordinator needs."
  (:require [texmachmfg.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (texmachmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (texmachmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :weaving-loom
                :model "WVL-220CM" :no-load-run-speed-rpm 600.0
                :quantity-units 1000.0 :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-units 200.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :sewing-machine
                :model "SWM-9000" :no-load-run-speed-rpm 5000.0
                :quantity-units 300.0 :defect-rate-percent 0.9
                :verified? true :registered? true
                :shipped-units 280.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :leather-cutting-machine
                :model "LCM-450" :no-load-run-speed-rpm 800.0
...
(defn- sample-equipment []
  {"loom-001" {:id "loom-001" :kind :loom-assembly-line
               :verified? true :registered? true
               :last-maintenance-date "2026-05-01"}
   "sew-002" {:id "sew-002" :kind :sewing-machine-test-bench
              :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `texmachmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `texmachmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  loom-assembly-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered sewing-machine-test-bench unit (blocks any
