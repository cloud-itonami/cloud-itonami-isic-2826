(ns texmachmfg.sim
  ...
  product type, a production-batch patch with an implausible no-load-
  run-speed reading, and a production-batch patch with an implausible
  defect-rate reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on loom-001 (verified, registered, loom assembly line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on loom-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on sew-002 (UNVERIFIED/unregistered sewing-machine test bench -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on loom-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible no-load-run-speed -> HARD hold ==")
