(ns texmachmfg.governor
  "Textile, Apparel and Leather Machinery Plant Operations Governor --
  the independent compliance layer that earns the TexMachAdvisor the
  ...
  `:textile-apparel-leather-machinery-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:loom/actuate` or
                                       `:test-bench/run`) is the
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`texmachmfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through.
   11. Invalid no-load-run-speed   -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:no-load-run-speed-rpm` that is
                                       not a physically plausible
                                       running-in test speed reading
                                       (`texmachmfg.registry/
                                       no-load-run-speed-rpm-valid?`),
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は織機・ミシン・皮革裁断機等の試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "織機・ミシン・皮革裁断機等の試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  under the EU Machinery Directive) -- an authority exclusively
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "機械安全認証(CE等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-no-load-run-speed-violations
  "For `:log-production-batch`, if the patch declares a
  `:no-load-run-speed-rpm` that is not a physically plausible
  running-in test speed reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rpm (:no-load-run-speed-rpm (:value proposal))]
      (when (and (some? rpm) (not (registry/no-load-run-speed-rpm-valid? rpm)))
        [{:rule :invalid-no-load-run-speed
          :detail (str rpm "rpm は物理的に妥当な無負荷試運転速度の範囲外")}]))))
...
(defn check
  "Censors a TexMachAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-no-load-run-speed-violations request proposal)
