(ns texmachmfg.operation
  "TexMachOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (TexMachAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Textile, Apparel and
  Leather Machinery Plant Operations Governor (:govern) and the
...
(defn build
  "Compiles a TexMachOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `texmachmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TexMachAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Textile, Apparel and Leather Machinery Plant Operations Governor -- independent censor.
      (g/add-node :govern
