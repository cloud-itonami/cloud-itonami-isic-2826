(ns texmachmfg.advisor
  "TexMachAdvisor -- the *contained intelligence node* for the
  textile/apparel/leather production-machinery (weaving looms,
  industrial sewing machines, leather-cutting machines, knitting
  machines and fabric-cutting machines) plant-operations coordination
  actor.

  It normalizes production-batch patches (product-type/no-load-run-
  ...
  a safety-concern flag, and drafts an outbound textile/apparel/
  leather production-machinery shipment coordination proposal against
  ...
  never a committed record and NEVER a real assembly/test-bench-
  equipment actuation, freight dispatch, or machinery safety
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `texmachmfg.governor` before
  ...
  `texmachmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  no-load-run-speed, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  ...
  `texmachmfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a mechanical-safety/electrical-safety/CE-compliance concern.
  ...
  technicality). See `texmachmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `texmachmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound textile/apparel/leather production-machinery
  shipment coordination proposal against a production batch. The
  advisor passes through the caller's own claimed unit quantity -- it
  does NOT invent one, and `texmachmfg.governor` NEVER trusts it: it
...
(def ^:private system-prompt
  (str "あなたは繊維機械・縫製機械・皮革裁断機械(織機・工業用ミシン・"
       "皮革裁断機等の繊維/アパレル/皮革生産用機械)製造工場"
       ...
       "織機・ミシン・皮革裁断機等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "機械安全認証(CE等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `texmachmfg.governor`
