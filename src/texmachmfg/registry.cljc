(ns texmachmfg.registry
  "Pure-function domain logic for the textile/apparel/leather
  production-machinery plant-operations coordination actor --
  equipment/batch verification, shipment-quantity recompute,
  product-type validation, no-load-run-speed plausibility validation,
  ...
  pre-existing `kotoba-lang/texmachmfg`-style capability library to
  ...
  `powertoolmfg.registry/shipment-quantity-exceeded?` from
  `cloud-itonami-isic-2818`, and `elecequipmfg.registry` from
  ...
  SCOPE: ISIC 2826 covers manufacture of machinery for textile,
  apparel and leather production -- weaving-loom, sewing-machine and
  leather-cutting-machine assembly and testing lines producing
  finished weaving looms, industrial sewing machines, leather-cutting
  machines, knitting machines and fabric-cutting machines. This actor
  ...
  (e.g. CE marking under the EU Machinery Directive).")
...
(def valid-product-types
  ...
  through."
  #{:weaving-loom :sewing-machine :leather-cutting-machine
    :knitting-machine :fabric-cutting-machine})

(def no-load-run-speed-rpm-min
  "Physical floor for a batch's own no-load running-in test speed
  reading, in rpm. A running-in test speed is never negative."
  0.0)

(def no-load-run-speed-rpm-max
  "Physical ceiling for a batch's own no-load running-in test speed
  reading, in rpm. Informed by typical no-load/running-in test speeds
  across this vertical's own equipment classes (weaving looms
  approximately 200-800 rpm, industrial sewing machines up to
  approximately 5,500 stitches/minute, leather-cutting-machine blade-
  stroke rates typically below 1,000 strokes/minute) and by the
  general mechanical-safety framework the ISO 11111 series (Textile
  machinery -- Safety requirements) establishes for this equipment
  class -- this ceiling generously bounds well above any real routine
  test on any standardized machine in this vertical; a reading above
  it is implausible sensor/QC data, not a real test."
  8000.0)
...
(defn batch-verified?
  ...
  (i.e. its product-type/no-load-run-speed/quantity/defect-rate claims
...
(defn no-load-run-speed-rpm-valid?
  "Is `rpm` a physically plausible no-load running-in test speed
  reading, in rpm? Rejects nil, non-numbers, negative values, and
  values beyond `no-load-run-speed-rpm-max` -- a fabricated or
  sensor-error reading, never let through as a real test-result fact."
  [rpm]
  (and (number? rpm)
       (>= (double rpm) no-load-run-speed-rpm-min)
       (<= (double rpm) no-load-run-speed-rpm-max)))
...
(defn register-maintenance
  ...
  the RECORD a plant coordinator would keep. `texmachmfg.governor`
...
(defn register-shipment
  ...
  outbound textile/apparel/leather production-machinery shipment
  against a verified, registered production batch. Pure function --
  does not dispatch any real freight carrier; it builds the RECORD a
  plant coordinator would keep. `texmachmfg.governor` independently
